...
**项目简介**
---
本项目旨在通过 **PyTorch** 与 **PySide6** 的结合，系统性地学习与实践 **卷积神经网络（CNN）** 的原理、结构与可视化过程。
项目不仅包含核心的深度学习模型训练代码，还通过图形化界面展示训练过程、损失变化、模型预测与可视化结果，帮助更直观地理解 CNN
的运行机制。

**数据描述**
---
[FashionMNIST](https://www.kaggle.com/datasets/zalando-research/fashionmnist) 是一个替代经典 MNIST 手写数字数据集的现代数据集，由
Zalando 的研究团队创建。它包含了 10 个类别的时尚单品灰度图像，是计算机视觉和机器学习领域的标准基准数据集。
